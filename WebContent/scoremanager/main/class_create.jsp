...
<c:import url="/common/base.jsp" >
	...
	<c:param name="content">
		...
		<c:if test="${not empty errors['1']}">
			...
			<div class="mt-2 text-warning">${errors["1"]}</div>
		</c:if>
		<c:if test="${not empty errors['2']}">
			...
			<div class="mt-2 text-warning">${errors["2"]}</div>
				...
				<div>
					...
					<input class="form-control" type="text" id="class_num" name="class_num" value="${class_num}" required maxlength="3" placeholder="クラス番号を入力してください" />
				</div>
